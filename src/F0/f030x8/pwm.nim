...
proc timer3seup() {.inline.} =
    ...
    TIM3.CR1.bset(TIM_CR1_CEN_Pos, TIM_CR1_ARPE_Pos)   # (CEN,ARPE) start countup
...
proc initPwm*() {.inline.} =
    ...
    block: # setup output port and pwm
        ...
        # PB4,[AF1],TIM3_CH1/PWM: Right Hi
        # PB5,[AF1],TIM3_CH2/PWM: Left  Hi
        GPIOB.MODER.bset(GPIO_MODER_MODER4_Pos + 1,GPIO_MODER_MODER5_Pos + 1)
        # [AF1] Select function -> PWM CHx
        GPIOB.AFR[0].bset(GPIO_AFRL_AFSEL4_Pos,GPIO_AFRL_AFSEL5_Pos)

        when PWM16BIT:
            RCC.AHBENR.bset(RCC_AHBENR_GPIOCEN_Pos)    # GPIOC clock enable
            # PC8,[AF0],TIM3_CH3/PWM
            # PC9,[AF0],TIM3_CH4/PWM
            GPIOC.MODER.bset(GPIO_MODER_MODER8_Pos + 1,GPIO_MODER_MODER9_Pos + 1)
            # [AF0] Select function -> PWM CHx
            # Default is AF0.
    #
    TIM3.CCER.st 0       # ch1..ch4 stop output. Inhibit during setting.
    # Start setting
    ...
    TIM3.EGR.bset(TIM_EGR_UG_Pos)     # (UG) set counter = 0 when overflow
