...
proc initSPI*(spi:auto) {.inline.} =
    if spi==SPI1:
        ...
        # AF5: select SPI1
        GPIOA.AFR[0].bset(  GPIO_AFRL_AFRL5_Pos, GPIO_AFRL_AFRL5_Pos + 2, #PA5
                            GPIO_AFRL_AFRL6_Pos, GPIO_AFRL_AFRL6_Pos + 2, #PA6
                            GPIO_AFRL_AFRL7_Pos, GPIO_AFRL_AFRL7_Pos + 2) #PA7

        GPIOA.OSPEEDR.bset( GPIO_OSPEEDER_OSPEEDR5_Pos,
                            GPIO_OSPEEDER_OSPEEDR5_Pos+1,
                            GPIO_OSPEEDER_OSPEEDR6_Pos,
                            GPIO_OSPEEDER_OSPEEDR6_Pos+1,
                            GPIO_OSPEEDER_OSPEEDR7_Pos,
                            GPIO_OSPEEDER_OSPEEDR7_Pos+1)
    ...
    spi.CR1.bset(SPI_CR1_BR_Pos) # sck= SYSTEM_CLOCK / 4
    # Data size 8bit
    spi.CR2.bset( SPI_CR2_DS_Pos,
                  SPI_CR2_DS_Pos + 1,
                  SPI_CR2_DS_Pos + 2)
