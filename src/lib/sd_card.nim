# Converted to nim language by audin, 2018/12.
#
...
var sd_byte_count: word
var sd_sector_count: word
...
const
    ...
    SD_SEND_STATUS {.used.} = 13

# -- Read Commands
const
    SD_SET_BLOCKLEN = 16
    SD_READ_SINGLE_BLOCK {.used.} = 17
    SD_READ_MULTIPLE_BLOCK = 18

# -- Write Commands
const
    SD_WRITE_BLOCK {.used.} = 24
    SD_WRITE_MULTIPLE_BLOCK {.used.} = 25
    SD_PROGRAM_CSD  {.used.}= 27

# -- Write Protection Commands
const
    SD_SET_WRITE_PROT {.used.} = 28
    SD_CLR_WRITE_PROT {.used.} = 29
    SD_SEND_WRITE_PROT {.used.} = 30

# -- Erase Commands
const
    SD_ERASE_WR_BLK_START {.used.} = 32
    SD_ERASE_WR_BLK_END {.used.} = 33
    SD_ERASE {.used.} = 38
...
const
    SD_READ_OCR = 58
    SD_CRC_ON_OFF {.used.} = 59

# -- application specific command, must write command 55 first
const
    SD_SD_STATUS {.used.} = 13
    SD_SEND_NUM_WR_BLOCKS {.used.} = 22
    SD_SET_WR_BLK_ERASE_COUNT {.used.} = 23
    SD_SD_APP_OP_COND = 41
    SD_SET_CLR_CARD_DETECT {.used.} = 42
    SD_SEND_SCR {.used.} = 51

# -- R1 RESPONCE boolS
const
    SD_IN_IDLE_STATE {.used.} = 0
    SD_ERASE_RESET {.used.} = 1
    SD_ILLEGAL_COMMAND = 2
    SD_COM_CRC_ERROR {.used.} = 3
    SD_ERASE_SEQUENCE_ERROR {.used.} = 4
    SD_ADDRESS_ERROR {.used.} = 5
    SD_PARAMETER_ERROR {.used.} = 6

var sd_error: bool
var sd_card_type: uint8
...
proc sd_get_number_of_sectors() {.inline.} =
    discard
...
proc sd_init*():bool {.inline.} =
    ...
    sd_cs_enable() #
    wait_ms(2) #            -- delay
    sd_cs_disable() #       -- chip select high
    for _ in 1..10:
    ...
    if  (response and (1 shl SD_ILLEGAL_COMMAND)) != 0:
        ...
        if response == 0xAA:
            ...
            sd_cs_disable() #   -- disable the sd card
            when SD_CARD_INFO:
                    xprintf("\nSucceed!: SD card init. ")
        else:
            when SD_CARD_INFO:
                xprintf("\nFailed!: ACMD41 command.\n")
            sd_error = true
            sd_cs_disable() #   -- disable the sd card
            sd_get_number_of_sectors()
            return false
