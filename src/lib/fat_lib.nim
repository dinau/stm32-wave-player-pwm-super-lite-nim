...
# --
var
    lgdwRootdir_sector: dword
    gdwTargetFileSector*:dword
...
proc searchNextFile*() =
    ...
    var dwRootdir_sector_size: dword
    var wNextDirEntryIndex {.global.}: word
    #  ; initialize
    ...
    when UART_INFO:
        ...
        fname[0] = lgbDirEntryBuff[0]  # To avoid calling memcopy() [large code size]
        fname[1] = lgbDirEntryBuff[1]
        fname[2] = lgbDirEntryBuff[2]
        fname[3] = lgbDirEntryBuff[3]
        fname[4] = lgbDirEntryBuff[4]
        fname[5] = lgbDirEntryBuff[5]
        fname[6] = lgbDirEntryBuff[6]
        fname[7] = lgbDirEntryBuff[7]
        fname[8] = '.'.byte
    ...
    when FILE_ENTRY_INFO:
        xprintf("\nlgdwBPB_FileSize(Filesize)=%d",lgdwBPB_FileSize)

    dwRootdir_sector_size = (32.dword * (dword)(lgwBPB_RootEntCnt) +
